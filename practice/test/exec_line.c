...
int launch(line *command_line, int th_of_command)
{
    ...
    if (pipe_command(command_line, th_of_command, block_array) < 0) {
...
int exec_builtin_cmd(char **block_array)
{
    ...
    //         return 1;//(*builtin_func[i])(block_array);
    //     }
    // }
    return -1;
}
...
int pipe_command(line *command_line, int th_of_command, char **block_array)
{
    ...
    if (pid == 0) {
        ...
        launch(command_line->next_line, th_of_command+1);
    }
    if (pid < 0) {
        // fork error
